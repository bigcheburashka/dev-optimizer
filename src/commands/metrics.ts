...
async function collectMetrics(projectPath: string): Promise<Metrics> {
...
async function collectDockerMetrics(projectPath: string): Promise<{
  imageSize: number;
  buildTime: number;
  layerCount: number;
  contextSize: number;
...
async function collectNpmMetrics(projectPath: string): Promise<{
...
function compareMetrics(baseline: Metrics, current: Metrics): Record<string, { 
  baseline: number; 
  current: number; 
  change: number;
  percent: number;
...
function formatComparison(
  ...
  for (const [metric, data] of Object.entries(comparison)) {
    ...
    lines.push(
      `${metric.padEnd(25)} | ${String(data.baseline).padStart(8)} | ${String(data.current).padStart(7)} | ${status} ${String(data.change).padStart(5)} | ${data.percent}%`
    );
  }
